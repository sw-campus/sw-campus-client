...
export function PostCard({ post, onClick }: PostCardProps) {
  return (
    <article
      ...
      className="group relative box-border h-[120px] max-h-[120px] min-h-[120px] w-full cursor-pointer overflow-hidden rounded-2xl border border-gray-100 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 active:scale-[0.98] md:h-[136px] md:max-h-[136px] md:min-h-[136px]"
    >
      <div className="absolute inset-0 flex gap-4 p-4">
        {/* 썸네일 */}
        <div className="relative h-[88px] w-[88px] flex-shrink-0 overflow-hidden rounded-xl bg-linear-to-br from-gray-50 to-gray-100 md:h-[104px] md:w-[104px]">
          {post.thumbnailUrl ? (
            <Image
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="h-[44px] flex-shrink-0 overflow-hidden md:h-[52px]">
            <h3 className="line-clamp-2 text-base font-semibold leading-[22px] text-gray-900 transition-colors group-hover:text-primary md:text-lg md:leading-[26px]">
              {post.title}
            </h3>
          </div>
      ...
      {/* 호버 효과 그라데이션 */}
      <div className="pointer-events-none absolute inset-0 bg-linear-to-r from-primary/0 via-primary/0 to-primary/5 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
    </article>
  )
}
