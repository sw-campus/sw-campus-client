...
export function DeviceDonutChart({ data, isLoading }: DeviceDonutChartProps) {
  ...
  return (
    <div className="bento-card group relative h-full overflow-hidden">
      ...
      {/* Decorative glow */}
      <div className="absolute top-1/2 left-1/2 h-24 w-24 -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-500/5 blur-2xl md:h-32 md:w-32" />
      ...
      <div className="relative z-10 flex h-full flex-col p-3 md:p-5">
        ...
        <div className="mb-2 flex items-center gap-2 md:mb-3 lg:mb-4">
          <div className="rounded-lg bg-blue-500/10 p-1.5 md:p-2">
            <LuMonitor className="h-3.5 w-3.5 text-blue-600 md:h-4 md:w-4" />
          </div>
          <h3 className="text-foreground text-xs font-bold md:text-sm lg:text-base">기기별 접속</h3>
        </div>
        ...
        {chartData.length > 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 md:gap-4">
            ...
            <div className="relative h-28 w-28 md:h-36 md:w-36 lg:h-44 lg:w-44">
                      ...
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="rounded-lg border border-white/20 bg-popover/95 px-2 py-1.5 shadow-xl backdrop-blur-sm md:rounded-xl md:px-3 md:py-2">
                            ...
                            <p className="font-mono-data text-foreground mt-0.5 text-base font-bold md:mt-1 md:text-lg">
                              {data.value.toLocaleString()}
                              <span className="text-muted-foreground ml-1 text-[10px] font-normal md:text-xs">
              ...
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-mono-data text-foreground text-lg font-bold md:text-2xl lg:text-3xl">{total.toLocaleString()}</span>
                <span className="text-muted-foreground text-[9px] md:text-[10px] lg:text-xs">총 접속</span>
              </div>
            </div>

            {/* Legend */}
            <div className="flex w-full flex-col gap-1 md:gap-1.5 lg:gap-2">
              {chartData.map(item => {
                ...
                return (
                  <div
                    key={item.key}
                    className="group/item flex items-center justify-between rounded-md bg-card/50 px-2 py-1.5 transition-all hover:bg-card/80 md:rounded-lg md:px-3 md:py-2"
                  >
                    <div className="flex items-center gap-1.5 md:gap-2">
                      <div className={cn('rounded p-1 md:rounded-md md:p-1.5', item.config.bg)}>
                        <Icon className={cn('h-2.5 w-2.5 md:h-3 md:w-3 lg:h-3.5 lg:w-3.5', item.config.text)} />
                      </div>
                      <span className="text-foreground text-[10px] font-medium md:text-xs lg:text-sm">{item.name}</span>
                    </div>
                    <div className="flex items-center gap-1.5 md:gap-2">
                      <span className="font-mono-data text-foreground text-xs font-bold md:text-sm lg:text-base">
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2">
            <div className="bg-muted rounded-full p-2 md:p-3">
              <LuMonitor className="text-muted-foreground h-5 w-5 md:h-6 md:w-6" />
            </div>
