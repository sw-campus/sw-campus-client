 ...
 */
export async function getAdjacentPosts(postId: number): Promise<import('./post-api.types').AdjacentPosts> {
  const { data } = await api.get<import('./post-api.types').AdjacentPosts>(`/posts/${postId}/adjacent`)
  return data
}
