  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2 p-3 sm:gap-2.5 sm:p-4">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto scrollbar-hide sm:gap-2">
            ...
            {isPopular && !post.pinned && (
              <span className="inline-flex shrink-0 items-center gap-0.5 rounded-md bg-gradient-to-r from-rose-500 to-pink-500 px-1.5 py-0.5 text-[10px] font-bold text-white shadow-sm sm:gap-1 sm:rounded-lg sm:px-2 sm:py-1 sm:text-xs">
                <FiTrendingUp className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                인기
              </span>
            )}
            <span className="shrink-0 rounded-md bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-600 sm:rounded-lg sm:px-2 sm:py-1 sm:text-xs">
              {post.categoryName}
            </span>
            {post.categoryName === BOOTCAMP_DIARY_CATEGORY_NAME && weekTag && (
              <span className="shrink-0 rounded-md bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium text-blue-600 ring-1 ring-blue-100 sm:rounded-lg sm:px-2 sm:py-1 sm:text-xs">
                {weekTag}
              </span>
            )}
          </div>
          ...
          {/* 통계 - 데스크탑에서만 상단에 표시 */}
          <div className="hidden shrink-0 items-center gap-3 text-xs text-gray-400 sm:flex">
        ...
        <div className="flex items-center justify-between gap-2">
          {/* 작성자 정보 */}
          <div className="flex items-center gap-1.5 text-[11px] text-gray-500 sm:gap-2 sm:text-xs">
            <UserAvatar nickname={post.authorNickname} size="xs" />
            <span className="max-w-[70px] truncate font-medium text-gray-700 sm:max-w-[100px]">{post.authorNickname}</span>
            <span className="text-gray-300">·</span>
            <span className="flex shrink-0 items-center gap-0.5">
              <FiClock className="hidden h-3 w-3 sm:block" />
              {relativeTime}
            </span>
          </div>
