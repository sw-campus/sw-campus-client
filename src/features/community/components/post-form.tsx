...
import {
  ...
  parseWeekTag,
  type DiaryFormData,
} from './diary-template-form'
import { DiaryTitleInput, buildDiaryTitle, buildWeekTag } from './diary-title-input'
import { LectureSearchModal, type SelectedLecture } from './lecture-search-modal'
        ...
        try {
          ...
          if (result.content && result.content.length > 0) {
            ...
            const matchedLecture =
              result.content.find(lecture => lecture.lectureName === lectureName && lecture.orgName === orgName) ||
              result.content[0]
  ...
  return (
    <>
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5 md:space-y-6">
        {/* 카테고리 선택 - Step 1 */}
        <div className="border-primary/30 from-primary/5 hover:border-primary/40 relative rounded-2xl border-2 bg-linear-to-br to-white p-4 shadow-sm transition-all hover:shadow-md sm:p-5">
          {/* 단계 표시 */}
          <div className="bg-primary text-primary-foreground absolute -top-3 left-4 flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold shadow-sm">
            <span>STEP 1</span>
          </div>
          <label className="mt-1 mb-3 flex items-center gap-2 text-sm font-semibold text-gray-800">
            <div className="bg-primary/10 flex h-7 w-7 items-center justify-center rounded-lg shadow-sm">
              <FiFolder className="text-primary h-3.5 w-3.5" />
            </div>
        ...
        {isBootcampDiaryCategory ? (
          ...
          <div className="border-primary/30 from-primary/5 hover:border-primary/40 relative rounded-2xl border-2 bg-linear-to-br to-white p-4 shadow-sm transition-all hover:shadow-md sm:p-5">
            ...
            <label htmlFor="title" className="mt-1 mb-3 flex items-center gap-2 text-sm font-semibold text-gray-800">
              <div className="bg-primary/10 flex h-7 w-7 items-center justify-center rounded-lg shadow-sm">
                <FiType className="text-primary h-3.5 w-3.5" />
              </div>
            ...
            <Input
              ...
              className="focus:border-primary/50 focus:ring-primary/20 h-11 rounded-xl border-gray-200 bg-gray-50/50 transition-all focus:bg-white focus:ring-2"
            />
        ...
        {isBootcampDiaryCategory ? (
          ...
          <div className="border-primary/30 from-primary/5 hover:border-primary/40 relative rounded-2xl border-2 bg-linear-to-br to-white p-4 shadow-sm transition-all hover:shadow-md sm:p-5">
            ...
            <label htmlFor="body" className="mt-1 mb-3 flex items-center gap-2 text-sm font-semibold text-gray-800">
              <div className="bg-primary/10 flex h-7 w-7 items-center justify-center rounded-lg shadow-sm">
                <FiFileText className="text-primary h-3.5 w-3.5" />
              </div>
        ...
        <div className="relative rounded-2xl border-2 border-dashed border-gray-200/80 bg-linear-to-br from-gray-50/30 to-white p-4 shadow-sm transition-all hover:border-gray-300/80 hover:bg-white hover:shadow-md md:p-5">
          ...
          <label htmlFor="tags" className="mt-1 mb-3 flex items-center gap-2 text-sm font-semibold text-gray-800">
            ...
            {selectedLecture && (
              <span className="bg-success/15 text-success ml-1 rounded-full px-2 py-0.5 text-xs font-medium">
                + 강의 정보 자동 추가
              </span>
            )}
          </label>
          ...
          {selectedLecture && (
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="bg-primary/10 text-primary ring-primary/20 inline-flex items-center gap-1 rounded-full px-3 py-1 text-sm font-medium ring-1">
        ...
        <div className="flex justify-end pt-2">
          <Button
            ...
            className="bg-primary text-primary-foreground shadow-primary/30 hover:shadow-primary/40 h-12 w-full rounded-xl text-base font-semibold shadow-md transition-all hover:shadow-lg active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-gray-300 disabled:bg-none disabled:text-gray-500 disabled:shadow-none sm:h-11 sm:w-auto sm:px-10 sm:text-sm"
          >
