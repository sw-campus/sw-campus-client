...
export const mapLectureDetailToFormValues = (
  ...
  return {
    ...
    afterCompletion: typeof detail.afterCompletion === 'boolean' ? detail.afterCompletion : false,
    url: detail.url ?? null,
    lectureImageFile: null, // 기존 이미지는 URL로만 표시
    startAtDate: parseDate(detail.startAt) ?? new Date(),
    endAtDate: parseDate(detail.endAt) ?? new Date(),
