...
export default function LectureTabNav() {
  ...
  // ========================================
  const tabContent = (
    <nav className="no-scrollbar flex w-full items-center gap-1.5 overflow-x-auto px-3 py-2.5 md:gap-4 md:px-6 md:py-3">
      {TABS.map(tab => (
        <button
          key={tab.id}
          ...
          type="button"
          className={cn(
            'group relative shrink-0 rounded-xl px-3 py-2.5 text-sm transition-all duration-200 md:px-5 md:py-2 md:text-lg',
            activeTab === tab.id
